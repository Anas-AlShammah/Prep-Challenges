...
const employeesBonus = (arr) => {
    arr.forEach((e) => {
        let a = e.salary.slice(0, e.salary.length - 1);
        a = a - '0';
        if (e.workHours > 8) {
        e.salary=a+100+'$';    
        }else{
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    mouseArray.sort((a,b)=>b-a);
    keyBoardArray.sort((a,b)=>b-a);
    let max=0;
    mouseArray.forEach(m=>{
        keyBoardArray.forEach(k=>{
            if (m+k<=budget && m+k>max){
                max=m+k;
            }
        }) 
    })
