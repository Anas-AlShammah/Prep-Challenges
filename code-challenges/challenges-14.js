...
const replaceWords = (str) => {
    let array=str.split(' ')
    let a=array.indexOf('I');
    let b=array.indexOf('am');
    let c=array.indexOf('was');
    if (a != -1){
        array.splice(a,1,'We')
    }
    if (b != -1){ array.splice(b,1,'are')}
    if (c != -1){ array.splice(c,1,'were')}
    return array.join(' ');
...
const arrToStr = (arr) => {
    let a=arr.length;
    for (let i=4;i<arr.length;i=i+5){
        arr[i]=arr[i]+","
    }
    return arr.join(' ');
}
